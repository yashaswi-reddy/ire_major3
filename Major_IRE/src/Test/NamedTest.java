...
public class NamedTest {
	...
	public NamedTest(String s,String s2)
	{
		dataFileName=s;	
		 linksFileName=s2;
	}
     ...
     for (Entry<String,String []> entry : test.entrySet())
       {
       	 ...
       	 FileWriter fw = new FileWriter("/media/82CC3BADCC3B9A7D/major_project/IRData/test-features/named_entities/"+key+".txt", false);
    	...
    	named_entities =abc.Named_entities(text);
    	 String [] token = named_entities.split("\t");
         for(int kk=0;kk< token.length ;kk++)
         {
        	 token[kk]=token[kk].toLowerCase();
        	 if(!(token[kk].equals("")) && !(token[kk].equals("volvo")))
        	 {
        		 file_out.write(token[kk]);
        		 file_out.newLine();
        	 }
         }
	...
	{
		NamedTest c = new NamedTest("/media/82CC3BADCC3B9A7D/major_project/IRData/Testing/tweet_text_modified/RL2013D01E003_texts.tsv","/media/82CC3BADCC3B9A7D/major_project/IRData/Testing/tweet_info/RL2013D01E003.dat");
		c.text();
	}
