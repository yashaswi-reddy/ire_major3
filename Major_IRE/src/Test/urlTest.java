...
public class urlTest {
	...
	public urlTest(String s,String s2)
	{
		dataFileName=s;	
		 linksFileName=s2;
	}
     ...
     for (Entry<String,String []> entry : test.entrySet())
       {
       	 ...
       	 FileWriter fw = new FileWriter("/media/82CC3BADCC3B9A7D/major_project/IRData/entity_8/test-features/url/"+key+".txt", false);
         BufferedWriter file_out= new BufferedWriter(fw);
       	 ...
       	 for(int i1=0;i1<url_list.length;i1++)
      	 {
       	 ...
       	 if(url_list[i1]!=null && !(url_list[i1].equals("")))
       	 {
       		 try
       		 {
       			File f = new File("/media/82CC3BADCC3B9A7D/major_project/IRData/Testing/downloaded_external_links/RL2013D01E008/"+url_list[i1]+"/1");
       			if(f.exists() && !f.isDirectory()) { 
			 ArrayList<Map.Entry> a = c.ans("/media/82CC3BADCC3B9A7D/major_project/IRData/Testing/downloaded_external_links/RL2013D01E008/"+url_list[i1]+"/1");
			 int l=0;
			 for (Map.Entry e : a)
				 {
					 String s=(String) e.getKey();
					 Integer v =(Integer)e.getValue();
					//  if ( !(s.toLowerCase().equals("toyota")))
					 if ( !(s.toLowerCase().equals("volkswagen")))
					  {
						  l++;
						  file_out.write(s);
						  file_out.newLine();
	...
	{
		urlTest c = new urlTest("/media/82CC3BADCC3B9A7D/major_project/IRData/Testing/tweet_text_modified/RL2013D01E008_texts.tsv","/media/82CC3BADCC3B9A7D/major_project/IRData/Testing/tweet_info/RL2013D01E008.dat");
		c.text();
	}
